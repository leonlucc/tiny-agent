...
>
> 本章是 Tiny Agent 的起点：不用前端、不做多轮历史，只先跑通“用户输入一句话，大模型回复一句话”的最小闭环。
...
本版本相对于空工程，核心变化是新增 `backend` 目录，上文已经详细讲解，完整代码已提交至 GitHub 仓库，你可以切换到分支 `v0.0.1` 查看或直接运行 `git checkout v0.0.1`查看。
