...
设置 `stream=True` 后，文本通常位于 `choices[0].delta.content`，而不再是完整响应中的 `choices[0].message.content`。chunk 是网络和模型服务决定的传输片段，可能是一个字、一个词，也可能是一小段文本，不能假设“一次 chunk 就等于一个字符”。页面看起来像逐字打印，本质上是收到一批就更新一批。

很多推理模型还会通过非标准字段 `choices[0].delta.reasoning_content` 返回思考片段。我们会把两类数据映射为不同业务事件：
...
如果要实现一个后端的流式接口，其核心是不一次性返回结果，而是逐次 `yield`。例如下面的`stream_chat_events()` ，通过`yield` + `async`的方式定义了一个异步生成器，允许调用方按需对异步请求而得到的数据进行分批迭代：

```python
async def stream_chat_events():
    ... # 先发起一个异步流式请求，得到 response
    async for chunk in response:
        ... # 解析数据等处理，等待时自动让出控制权
...
>
> 在 Python 和 JavaScript 中，`yield` 就像一个可暂停的 `return`：交出值后函数会记住当前位置，下次取值时从暂停处继续执行，因此带 `yield` 的函数就是生成器，天生适合“边产生、边消费”的数据序列。`yield` 本身不创建并发，只负责把数据分批交出；真正的异步能力来自 `async/await`——它让函数在等待 I/O 等操作时挂起并释放控制权，不阻塞其他任务。将 `yield` 与 `async` 结合便得到异步生成器（即异步迭代器），可分别用 Python 的 `async for` 和 JavaScript 的 `for await...of` 逐项消费。
...
`data:` 是 SSE 定义的字段名，每个事件以空行结束，也就是字符串中的 `\n\n`。
`{"type":"...","chunk":"..."}` JSON数据， 是项目选择的业务载荷格式，不是 SSE 强制规定的格式；
...
`backend/app/main.py` 从 CLI 入口变成 Web 应用的装配点。`create_app()` 注册 API、挂载静态页面，并把 `lifespan` 交给 FastAPI：
...
- **原生采用 ASGI 和异步接口**：路由函数可以直接使用 `async def`，自然衔接 `AsyncOpenAI`、异步生成器和 `StreamingResponse`，不需要在线程池与异步事件循环之间额外转换。
- **流式响应表达直接**：把异步生成器交给 `StreamingResponse`，每次 `yield` 的 SSE 帧就能继续向下游传递，代码结构与“边生成、边发送”的数据流一致。
- **请求校验简洁**：使用 Pydantic 模型声明 `ChatRequest` 后，FastAPI 会完成 JSON 解析和基础类型校验，接口代码可以专注于空消息检查与流式响应。
- **工程规模合适**：本章只需要两个 API 和静态文件托管，不需要完整的后台管理、ORM、模板系统或复杂的项目脚手架。

这并不意味着 FastAPI 在所有 Web 项目中都优于其他框架。Flask 同样适合构建小型服务，但它更偏向同步请求模型；要把本章的异步 LLM 调用和流式输出贯穿到底，需要做更多运行方式上的选择。Django 提供 ORM、认证、后台管理等完整能力，适合功能更丰富的业务系统，但对当前最小示例来说会引入许多暂时用不到的概念。Starlette 或 aiohttp 也能很好地处理异步流，不过请求模型、数据校验和接口组织通常需要编写更多基础代码。

因此，这里的选择标准不是“哪个框架最强”，而是**哪个框架能用最少的额外概念，清楚地呈现本章的异步流式链路**。如果项目未来发展出复杂的数据模型、后台管理或既有技术栈约束，重新评估 Django、Flask 或其他 ASGI 框架也是合理的。
...
### 6.3 为什么前端使用 `fetch()`，而不是 `EventSource`？
...
### 6.4 为什么定义业务事件，而不原样转发 SDK chunk？
