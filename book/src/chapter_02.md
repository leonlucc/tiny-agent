...
> **导语**：上一章已经让 Tiny Agent 在终端中完成了一次 LLM 调用。本章把这条同步、整段返回的 CLI 链路升级为可在浏览器中实时阅读的流式链路：后端一边接收模型输出，一边通过 SSE 转发；前端一边解析事件，一边更新页面。
>
...
因此，本章的目标不是做一个功能完整的聊天产品，而是建立最小的实时 Web 闭环：**输入一条消息，模型回复随生成过程逐步出现在网页上**。本版本仍然是单轮调用，不保存聊天历史，也不让模型记住上一轮内容。

---

## 2. 整体方案

v0.2 在上一章 LLM 调用的外面增加两层：FastAPI Web 服务和原生浏览器页面。后端同时负责 API 路由与静态文件托管，因此页面和接口来自同一个服务，不需要额外配置跨域访问。

用户发送消息后，浏览器使用 `POST /api/chat/stream` 提交 JSON。后端把消息交给 LLM 服务层，并开启 `stream=True`。模型每产生一段增量内容，服务层就把它转换为与厂商响应格式解耦的业务事件；接口层再把业务事件编码成 SSE 帧。浏览器持续读取响应流，解析出 `reasoning`、`content` 和 `error` 事件，最终增量更新页面。
...
7. 应用层累积文本并更新同一个助理消息节点；收到 `[DONE]` 后恢复输入区。
...
本节只介绍跑通本章链路必需的知识：LLM 增量输出、异步生成器、SSE 帧格式，以及浏览器端的流式解码。

### 3.1 完整响应与增量响应

上一章调用模型时，SDK 返回的是已经生成完毕的完整响应：
...
1. 展示用户消息，清空并锁定输入区。
2. 创建“正在思考”指示器和隐藏的助理消息节点。
3. 发起流式请求，使用 `for await...of` 消费 SSE 事件。
4. 分别累积 `reasoning` 和 `content`，再更新助理消息。
5. 正常结束时完成消息；空流或异常时展示错误。
6. 在 `finally` 中恢复输入区并重新聚焦。

`appState.isTyping` 保证同一时间只有一个流式请求。这个限制让 v0.2 不必处理多个响应交错、取消请求或消息归属等额外状态。

`frontend/js/components/chat-ui.js` 只负责局部交互与 DOM 更新。它通过 `createAssistantResponseView()` 返回 `update`、`complete`、`dispose` 三个操作，把一次助理回复看成一个 UI 生命周期。文本使用 `textContent` 写入，而不是直接拼接 `innerHTML`，既能保留纯文本语义，也避免把模型输出当作可执行 HTML 注入页面。
...
在输入框中发送一条消息。如果“正在思考”提示随后被助理消息替换，并且回复内容持续增长，说明从模型到浏览器的流式链路已经跑通。

> **系统页面截图（待补充）**
>
> 此处预留 v0.2 Streaming Web 运行效果截图，截图应包含输入框、已发送的用户消息，以及正在流式生成的助理回复。

---

## 5. Git Diff 导读

从上一章的 CLI 基线看，v0.2 的能力变化可以归纳为四组：
...
该范围包含 16 个文件，共新增 1106 行、删除 93 行。关键变化是：

- 删除 `backend/app/simple_call.py`，结束 CLI 直接调用模式。
- 重写 `backend/app/main.py`，将程序入口改为 FastAPI Web 服务。
- 新增 `config.py`、`api/` 和 `service/`，拆分配置、传输与模型调用职责。
- 新增 `frontend/` 页面及其样式、API、SSE 解析和 UI 模块。
- 在 `requirements.txt` 中加入 FastAPI 与 Uvicorn。
...
WebSocket 更适合双方都要随时主动推送的长连接场景，例如语音对话、协同编辑或服务端主动通知。它当然也能完成本章任务，但会额外引入连接生命周期、消息路由和心跳等概念，不符合 v0.2 的最小目标。

### 6.2 为什么使用 `fetch()`，而不是 `EventSource`？
...
`reasoning`、`content`、`error` 是 Tiny Agent 自己的最小事件协议。服务层负责吸收模型差异，接口层和前端只依赖稳定事件。这是一个很小但重要的反腐层：外部协议变化不会直接扩散到整个应用。
